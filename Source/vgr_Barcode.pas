{Contains the TvgrBarcode class which draws the barcode on the specified canvas. TvgrBarcode is a port of TvgrBarcode component; author - Andreas Schmidt.
}
unit vgr_Barcode;
...
{
==============================================================
Unit was ported into PReport and GridReport by
Alexander Manuzin, 22 May 2004

Unit name is changed from Barcode to vgr_Barcode,
added [vgr] prefix to all visible classes and types in this
mudule, it was done to avoid the naming conflicts.
==============================================================
...
- renamed TBarcode to TvgrBarcode to avoid name conflicts
...
uses
  Classes, Graphics{$IFDEF VTK_D6_OR_D7}, Types {$ELSE} ,Windows{$ENDIF};

type
{Describes the supported barcode types.}
  TvgrBarcodeType =
  (
...
{Only for internal use, specifies the various types of lines in the barcode.,
black_half means a black line with 2/5 height (used for PostNet).}
  TvgrBarLineType = (white, black, black_half);

{Describes the which text will be displayed on barcode.
Items:
  bcoNone - Text will be not displayed.
  bcoCode - The code of barcode will be displayed.
  bcoType - The string representation of type of barcode will be displayed.
  bcoBoth - Both code and type will be displayed.}
  TvgrBarcodeOption = (bcoNone, bcoCode, bcoTyp, bcoBoth); { Type of text to show }

// Additions from Roberto Parola to improve the text output
{Describes the position of text.}
  TvgrShowTextPosition =
  (
    stpTopLeft,
    stpTopRight,
    stpTopCenter,
...
{Describes the method of checksum calculating.}
  TvgrCheckSumMethod =
  (
  csmNone,
  csmModulo10
...
{Represents the measuring parameters of barcode.
Syntax:
  rvgrMeasureParams = record
    MaxTextSize: TSize;
    LinesWidth: Integer;
    LinesTop: Integer;
    LinesHeight: Integer;
  ...
  rvgrMeasureParams = record
    ...
    MaxTextSize: TSize;
    TextTop: Integer;
    TextRotatedHeight: Integer;
  end;

{This class draws the barcode on specified canvas.}
  TvgrBarcode = class(TObject)
  private
    FHeight: Integer;
    FText: string;
    FTop: Integer;
    ...
    FModul: Integer;
    FRatio: Double;
    FTyp: TvgrBarcodeType;
    FCheckSum: boolean;
    FShowText: TvgrBarcodeOption;
    FAngle: double;
    FColor: TColor;
    FColorBar: TColor;
    FCheckSumMethod: TvgrCheckSumMethod;
    FShowTextFont: TFont;
    FShowTextPosition: TvgrShowTextPosition;
    FShowTextOverLines: Boolean;
    FOnChange: TNotifyEvent;

    modules:array[0..3] of shortint;

    procedure OneBarProps(code:char; var Width:integer; var lt:TvgrBarLineType);
    ...
    procedure SetTyp(const Value: TvgrBarcodeType);
    procedure SetAngle(const Value: Double);
    procedure SetText(const Value: string);
    procedure SetShowText(const Value: TvgrBarcodeOption);
...
// Additions from Roberto Parola to improve the text output
    ...
    procedure SetShowTextPosition(const Value: TvgrShowTextPosition);
    procedure SetShowTextOverLines(Value: Boolean);

  protected
    procedure Measure(ACanvas: TCanvas; const ARect: TRect; AText: TStringList; var AParams: rvgrMeasureParams);
    function MakeData: string;
    procedure DoChange; virtual;

    function Rotate(const APoint: TPoint; const ARect: TRect; const AParams: rvgrMeasureParams): TPoint;
    procedure DoLines(data: string; Canvas: TCanvas; const ARect: TRect; const AParams: rvgrMeasureParams);
    procedure DrawText(Canvas: TCanvas; const ARect: TRect; AText: TStringList; const AParams: rvgrMeasureParams);
  public
{Creates an instance of the TvgrBarcode class.}
    constructor Create;
{Frees an instance of the TvgrBarcode class.}
    destructor Destroy; override;

{Copies the properties of another TvgrBarcode object.}
    procedure Assign(Source: TvgrBarcode);

{Performs the drawing of barcode including the text.
Parameters:
  Canvas - Specified the TCanvas object on which the barcode will be drawn.}
    procedure DrawBarcode(Canvas: TCanvas; const ARect: TRect);
{Indicates whether the barcode is correct and can be displayed.
See also:
  DrawBarcode}
    function Checkbarcode: Boolean;
    function GetLinesWidth: Integer;
{Returns the height of barcode on the canvas.}
...
{Width of Barcode in pixels.
The width of barcode is calculated automatically, when you change this property you change the value of
Modulo property - it will be adjusted to fit a barcode in the specified width.}
    property Width: integer read GetWidth write SetWidth stored False;
{The Y coordinate of the top-left corner of barcode.}
    property Top: Integer read FTop write SetTop;
{The X coordinate of the top-left corner of barcode.}
...
{Specifies the zoom factor of barcode.}
    property Ratio: Double read FRatio write SetRatio;
{Specifies the type of barcode.
See also:
  TvgrBarcodeType}
    property Typ: TvgrBarcodeType read FTyp write SetTyp default bcCode_2_5_interleaved;
{Indicates whether the checksum should be built.}
    property Checksum: Boolean read FCheckSum write SetCheckSum default FALSE;
{Specifies the method of checksum calculating.
See also:
  TvgrCheckSumMethod}
    property CheckSumMethod: TvgrCheckSumMethod read FCheckSumMethod write FCheckSumMethod default csmModulo10;

{Specifies the angle of rotation of barcode, can be: 0, 90, 180 or 270.}
    property Angle: Double read FAngle write SetAngle;
{Indicates whether the text should be displayed with barcode.
See also:
  ShowTextFont, ShowTextPosition}
    property ShowText: TvgrBarcodeOption read FShowText write SetShowText default bcoNone;
{Specifies the font of displayed text.}
    property ShowTextFont: TFont read FShowTextFont write SetShowTextFont;
{Specifies the position of text.
See also:
  TvgrShowTextPosition}
    property ShowTextPosition: TvgrShowTextPosition read FShowTextPosition write SetShowTextPosition default stpTopLeft;
{Specifies the background color of barcode.}
    property Color: TColor read FColor write FColor default clWhite;
{Specifies the color of lines of barcode.}
    property ColorBar: TColor read FColorBar write FColorBar default clBlack;
{Indicates whether the text should drawn over lines of barcode.}
    property ShowTextOverLines: Boolean read FShowTextOverLines write SetShowTextOverLines default False;
{This event occurs when new any of object properties is changed.}
...
const BCdata:array[TvgrBarcodeType] of TBCdata =
  (
...
constructor TvgrBarcode.Create;
begin
  inherited Create;
  FAngle := 0.0;
  FRatio := 2.0;
...
destructor TvgrBarcode.Destroy;
begin
  FShowTextFont.Free;
  inherited;
end;

procedure TvgrBarcode.Assign(Source: TvgrBarcode);
begin
  FHeight    := Source.FHeight;
  FText      := Source.FText;
  FTop       := Source.FTop;
  FLeft      := Source.FLeft;
  FModul     := Source.FModul;
  FRatio     := Source.FRatio;
  FTyp       := Source.FTyp;
  FCheckSum  := Source.FCheckSum;
  FShowText  := Source.FShowText;
  FShowTextPosition := Source.FShowTextPosition;  // 15.05.2003
  FAngle     := Source.FAngle;
  FColor     := Source.FColor;
  FColorBar  := Source.FColorBar;
  FCheckSumMethod := Source.FCheckSumMethod;
  FShowTextOverLines := Source.FShowTextOverLines;
  FOnChange  := Source.FOnChange;
end;


function TvgrBarcode.GetTypText:string;
begin
...
{ set Modul Width  }
procedure TvgrBarcode.SetModul(v:integer);
begin
...
procedure TvgrBarcode.OneBarProps(code:char; var Width:integer; var lt:TvgrBarLineType);
begin
  case code of
...
function TvgrBarcode.MakeData : string;
var
  i : integer;
begin
...
function TvgrBarcode.GetWidth:integer;
var
  data : string;
  i : integer;
  w : integer;
...
procedure TvgrBarcode.SetWidth(Value :integer);
var
  data : string;
  i : integer;
  w, wtotal : integer;
...
function TvgrBarcode.DoCheckSumming(const data : string):string;
begin
...
function TvgrBarcode.Code_EAN8:string;
var
  i : integer;
  tmp : String;
begin
...
function TvgrBarcode.Code_EAN13:string;
var
  i, LK: integer;
  tmp : String;
begin
...
function TvgrBarcode.Code_2_5_interleaved:string;
var
  i, j: integer;
  c : char;
...
function TvgrBarcode.Code_2_5_industrial:string;
var
  i, j: integer;
begin
...
function TvgrBarcode.Code_2_5_matrix:string;
var
  i, j: integer;
  c :char;
begin
...
function TvgrBarcode.Code_39:string;
...
function TvgrBarcode.Code_39Extended:string;
...
function TvgrBarcode.Code_128:string;
type TCode128 =
  record
    a, b : char;
    c : string[2];
...
function TvgrBarcode.Code_93:string;
type TCode93 =
  record
    c : char;
...
function TvgrBarcode.Code_93Extended:string;
...
function TvgrBarcode.Code_MSI:string;
...
function TvgrBarcode.Code_PostNet:string;
...
function TvgrBarcode.Code_Codabar:string;
type TCodabar =
  record
    c : char;
...
{Assist function}
function TvgrBarcode.SetLen(pI:byte):string;
begin
...
function TvgrBarcode.Code_UPC_A:string;
var
  i : integer;
  tmp : String;
begin
...
function TvgrBarcode.Code_UPC_E0:string;
var i,j : integer;
   tmp : String;
   c   : char;
begin
...
function TvgrBarcode.Code_UPC_E1:string;
var i,j : integer;
   tmp : String;
   c   : char;
begin
...
function TvgrBarcode.Code_Supp5:string;
var i,j : integer;
   tmp : String;
   c   : char;
begin
...
function TvgrBarcode.Code_Supp2:string;
var i,j : integer;
   tmp,mS : String;
begin
  FText := SetLen(2);
...
procedure TvgrBarcode.MakeModules;
begin
  case Typ of
    bcCode_2_5_interleaved,
    bcCode_2_5_industrial,
...
function TvgrBarcode.Rotate(const APoint: TPoint; const ARect: TRect; const AParams: rvgrMeasureParams): TPoint;
begin
  with AParams do
  begin
    Result.X := Round(APoint.X * Cos - APoint.Y * Sin);
    Result.Y := Round(APoint.X * Sin + APoint.Y * Cos);
  end;
  Result.X := Result.X + ARect.Left + (ARect.Right - ARect.Left) div 2;
  Result.Y := ARect.Top + (ARect.Bottom - ARect.Top) div 2 - Result.Y;
end;
...
procedure TvgrBarcode.DoLines(data: string; Canvas: TCanvas; const ARect: TRect; const AParams: rvgrMeasureParams);
var
  I: Integer;
  lt: TvgrBarLineType;
  xadd: Integer;
  width, height: Integer;
  a, b: TPoint;   {Edges of a line (we need 4 Point because the line}
                        {is a recangle}
  R: TRect;

begin
  xadd := AParams.LinesLeft;
    ...
    begin
      ...
      begin
        ...
        if lt = black_half then
          height := AParams.LinesHeight * 2 div 5
        else
          height := AParams.LinesHeight;
        height := height - 1;

        a.x := xadd;
        a.y := AParams.LinesTop;

        b.x := xadd + width;
        b.y := AParams.LinesTop - height;

        {rotate the rectangle}
        a := Rotate(a, ARect, AParams);
        b := Rotate(b, ARect, AParams);
...
// Measures the parameters of barcode
procedure TvgrBarcode.Measure(ACanvas: TCanvas; const ARect: TRect; AText: TStringList; var AParams: rvgrMeasureParams);
var
  I: Integer;
  ASize: TSize;
begin
  AParams.RotateAngle := FAngle * PI / 180;
  SinCos(AParams.RotateAngle, AParams.Sin, AParams.Cos);
  ...
  if FShowText in [bcoTyp, bcoBoth] then
    AText.Add(GetTypText());

  ACanvas.Font.Assign(FShowTextFont);
  AParams.MaxTextSize.cx := 0;
  AParams.MaxTextSize.cy := 0;
  for I := 0 to AText.Count - 1 do
  begin
    ASize := ACanvas.TextExtent(AText[I]);
    if ASize.cx > AParams.MaxTextSize.cx then
      AParams.MaxTextSize.cx := ASize.cx;
    AParams.MaxTextSize.cy := AParams.MaxTextSize.cy + ASize.cy; 
  end;

  AParams.LinesWidth := GetWidth;
  AParams.Width := Max(AParams.MaxTextSize.cx, AParams.LinesWidth);
  if Abs(AParams.Cos) > Abs(AParams.Sin) then
  begin
    // rotation across of Y axis
    AParams.LinesSize := ARect.Bottom - ARect.Top;
    AParams.RotateOffset := Round(Abs(AParams.Width * AParams.Sin / 2));
    AParams.TextRotatedHeight := Round(Abs(AParams.MaxTextSize.cy / AParams.Cos));
  end
  else
  begin
    // rotation across of X axis
    AParams.LinesSize := ARect.Right - ARect.Left;
    AParams.RotateOffset := Round(Abs(AParams.Width * AParams.Cos / 2));
    AParams.TextRotatedHeight := Round(Abs(AParams.MaxTextSize.cy / AParams.Sin));
  end;

  if AParams.MaxTextSize.cy > AParams.LinesSize then
  begin
    // do not show text, it does not fit
    AText.Clear;
    AParams.MaxTextSize.cx := 0;
    AParams.MaxTextSize.cy := 0;
    AParams.TextRotatedHeight := 0;
  end;

  if ShowTextPosition in [stpTopLeft, stpTopRight, stpTopCenter] then
  begin
    AParams.LinesTop := (AParams.LinesSize div 2) - (AParams.RotateOffset div 2);
    if not ShowTextOverLines then
      AParams.LinesTop := AParams.LinesTop - AParams.TextRotatedHeight;
    AParams.TextTop := (AParams.LinesSize div 2) - (AParams.RotateOffset div 2);
  end
  else
  begin
    AParams.LinesTop := (AParams.LinesSize div 2) - (AParams.RotateOffset div 2);
    AParams.TextTop := -((AParams.LinesSize div 2) - (AParams.RotateOffset div 2) - AParams.TextRotatedHeight);
  end;
  AParams.LinesLeft := - AParams.LinesWidth div 2;
  AParams.LinesHeight := AParams.LinesSize - AParams.RotateOffset;
  if not ShowTextOverLines then
    AParams.LinesHeight := AParams.LinesHeight - AParams.TextRotatedHeight;
end;

function TvgrBarcode.GetLinesWidth: Integer;
var
  DC: HDC;
  ACanvas: TCanvas;
  AParams: rvgrMeasureParams;
  AText: TStringList;
begin
  ...
  ACanvas.Handle := DC;
  try
    Measure(ACanvas, Rect(0, 0, 0, 0), AText, AParams);
    Result := AParams.Width;
  finally
    ACanvas.Free;
    ReleaseDC(0, DC);
    AText.Free;
...
function TvgrBarcode.Checkbarcode: Boolean;
begin
  try
    MakeData;
    Result := True;
...
procedure TvgrBarcode.DrawBarcode(Canvas: TCanvas; const ARect: TRect);
var
  data : string;
  SaveFont: TFont;
  SavePen: TPen;
...
begin
  ...
  GetViewportOrgEx(Canvas.Handle, AOrgEx);
  AClipRect := ARect;
  OffsetRect(AClipRect, AOrgEx.X, AOrgEx.Y);
  ANewRgn := CreateRectRgnIndirect(AClipRect);
  AOldRgn := CreateRectRgn(0, 0, 1, 1);
  if GetClipRgn(Canvas.Handle, AOldRgn) = 0 then
  begin
    DeleteObject(AOldRgn);
    AOldRgn := 0;
  end;
  SelectClipRgn(Canvas.Handle, ANewRgn);
  ...
  try
    ...
    // Alexander Manuzin:
    // calculate layout of object:
    //   - text to display
    //   - max text sizes (if it has more than one line)
    //   - height for lines of barcode
    Measure(Canvas, ARect, AText, AParams);

    DoLines(data, Canvas, ARect, AParams);
    if AText.Count > 0 then
      DrawText(Canvas, ARect, AText, AParams);   {show readable Text}
    ...
    SelectClipRgn(Canvas.Handle, AOldRgn);
    if AOldRgn <> 0 then
      DeleteObject(AOldRgn);
...
procedure TvgrBarcode.DrawText(Canvas: TCanvas; const ARect: TRect; AText: TStringList; const AParams: rvgrMeasureParams);
var
  S: string;
  I: Integer;
  ASize: TSize;
  APos, ARotatedPos: TPoint;
  ANewFont, AOldFont: HFONT;
  LF: TLogFont;
begin
  Canvas.Brush.Color := Color;
  Canvas.Font.Color := ColorBar;

  // create rotated font
  GetObject(ShowTextFont.Handle, SizeOf(TLogFont), @LF);
  LF.lfEscapement := Round(FAngle * 10);
  LF.lfOrientation := Round(FAngle * 10);
  ANewFont := CreateFontIndirect(LF);
  AOldFont := SelectObject(Canvas.Handle, ANewFont);

  APos.Y := AParams.TextTop;
  for I := 0 to AText.Count - 1 do
  begin
    S := AText[I];
    ...
    case ShowTextPosition of
      stpTopRight, stpBottomRight:
        APos.X := AParams.Width - ASize.cx;
      stpTopCenter, stpBottomCenter:
        APos.X := (AParams.Width - ASize.cx) div 2;
      else
        APos.X := 0;
    end;
    APos.X := APos.X - (AParams.Width div 2);

    // rotate the point and draw text
    ARotatedPos := Rotate(APos, ARect, AParams);
    ...
    //
    APos.Y := APos.Y - ASize.cy;
  end;

  // select old font and delete created font
  SelectObject(Canvas.Handle, AOldFont);
  DeleteObject(ANewFont);
end;


procedure TvgrBarcode.DoChange;
begin
  if Assigned(FOnChange) then
    FOnChange(Self);
end;

procedure TvgrBarcode.SetRatio(const Value: Double);
begin
...
procedure TvgrBarcode.SetTyp(const Value: TvgrBarcodeType);
begin
   if Value <> FTyp then
   begin
      FTyp := Value;
...
procedure TvgrBarcode.SetAngle(const Value: Double);
begin
...
procedure TvgrBarcode.SetText(const Value: string);
begin
   if Value <> FText then
   begin
      FText := Value;
...
procedure TvgrBarcode.SetShowText(const Value: TvgrBarcodeOption);
begin
...
procedure TvgrBarcode.SetTop(const Value: Integer);
begin
   if Value <> FTop then
   begin
      FTop := Value;
...
procedure TvgrBarcode.SetLeft(const Value: Integer);
begin
   if Value <> FLeft then
   begin
      FLeft := Value;
...
procedure TvgrBarcode.SetCheckSum(const Value: Boolean);
begin
...
procedure TvgrBarcode.SetHeight(const Value: integer);
begin
...
function TvgrBarcode.GetCanvasHeight: Integer;
var
  alpha :Extended;
begin
...
function TvgrBarcode.GetCanvasWidth: Integer;
var
  alpha :Extended;
begin
  alpha := FAngle/180.0*pi;
...
procedure TvgrBarcode.SetShowTextFont(const Value: TFont);
begin
  FShowTextFont.Assign(Value);
  DoChange;
end;

procedure TvgrBarcode.SetShowTextPosition(const Value: TvgrShowTextPosition);
begin
...
procedure TvgrBarcode.SetShowTextOverLines(Value: Boolean);
begin
  if Value <> FShowTextOverLines then
  begin
    FShowTextOverLines := Value;
    DoChange;
  end;
end;
